 ...
 */
__global__
void bodyMove(Body *p, float dt, int n) {
  int index = threadIdx.x + blockIdx.x*blockDim.x;
  int stride = blockDim.x * gridDim.x;
...
int main(const int argc, const char** argv) {
  ...
  status = cudaMallocManaged(&buf, bytes);
  if (status != cudaSuccess) printf("Error: %s\n", cudaGetErrorString(status));
   ...
   */
    ...
    bodyMove<<<numberOfBlocks,threadsPerBlock>>>(p, dt, nBodies);
    status = cudaGetLastError();
    if (status != cudaSuccess) printf("Error: %s\n", cudaGetErrorString(status));
