...
(define (stream-map proc . streamlist)
  (if (or (null? streamlist)
          (stream-null? (car streamlist)))
      the-empty-stream
      (cons-stream
        (apply proc (map stream-car streamlist))
        (apply stream-map
               (cons proc (map stream-cdr streamlist))))))
...
(define (display-stream seq)
  (if (not (stream-null? seq))
      (begin
        (display (stream-car seq))
        (newline)
        (display-stream (stream-cdr seq)))))
           ...
           (display-stream
            (stream-map
             (lambda (frame)
               (instantiate query
                            frame
                            (lambda (v f) ;; conjugate of query-syntax-process
  ...
  (define (copy exp)
    (cond ((var? exp)
           (let ((binding (binding-in-frame exp frame)))
             (if binding
                 (copy (binding-value binding)) ;; binding can be another bind
...
(define (qeval query frame-stream)
  (let ((qproc (get (type query) 'qeval)))
    (if qproc
        (qproc (contents query) frame-stream) ;; special forms
  ...
  (stream-flatmap
    (lambda (frame)
      (stream-append-delayed
        (find-assertions query-pattern frame) ;; match pattern
        (delay (apply-rules query-pattern frame)))) ;; apply all possible rules
    frame-stream))
